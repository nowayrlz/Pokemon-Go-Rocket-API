    ...
    public class Logic
    {
        ...
        private async Task ExecuteCatchAllNearbyPokemons(Client client)
        {
            ...
            foreach (var pokemon in pokemons)
            {
                ...
                System.Console.WriteLine(caughtPokemonResponse.Status == CatchPokemonResponse.Types.CatchStatus.CatchSuccess ? $"[{DateTime.Now.ToString("HH:mm:ss")}] We caught a {pokemon.PokemonId} with CP {encounterPokemonResponse?.WildPokemon?.PokemonData?.Cp} XP: { caughtPokemonResponse.Scores.Xp.Sum() } using a {pokeball}" : $"[{DateTime.Now.ToString("HH:mm:ss")}] {pokemon.PokemonId} with CP {encounterPokemonResponse?.WildPokemon?.PokemonData?.Cp} got away using a {pokeball}..");
                await Task.Delay(10000);
                await TransferDuplicatePokemon();
            }
        }
